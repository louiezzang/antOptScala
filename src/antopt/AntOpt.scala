...
class AntOpt(val nodes: Map[Int, Point], val numAnts: Int = 150, generations: Int = 200) {
  ...
  val edges = connections
    .map(
      connection =>
        (
          connection,
          EdgeData(r.nextDouble() * 0.1, nodes(connection.startNodeId).distance(nodes(connection.targetNodeId)).toInt)))
    .toMap[Edge, EdgeData]
  ...
  @tailrec
  private def adjustPheromones(tours: List[Tour],
                               changes: Map[Edge, EdgeData],
                               edges: Map[Edge, EdgeData]): Map[Edge, EdgeData] = {
    tours.size match {
      case 0 => createAdjustedEdges(edges, changes)
      case _ => {
        val adjustedChanges = changes
          .map(change => {
          ...
          .toMap
        val newChanges = tours.head.route
          .filter(!changes.contains(_))
          .map(segment => (segment, edges(segment).adjustPheromones(tours.head.length)))
          .toMap
    ...
    generation match {
      ...
      case _ => {
        ...
        if (shortestTour.length < bestTour.length) {
          println(
            "new shortest Tour in generation " + generation + ": " + shortestTour.length + " : " + shortestTour.route)
...
object AntOpt extends App {
  ...
  def getNodesFromFile(fileName: String) = {
    ...
    (for (i <- (0 to dim - 1))
      yield
        (i + 1, Point(coordSection.get(i).get("x").asString.toInt, coordSection.get(i).get("y").asString.toInt))).toMap
  }
